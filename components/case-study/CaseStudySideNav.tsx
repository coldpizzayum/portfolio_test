...
/**
 * Left-column TOC — a sticky sidebar, part of the page's normal
 * two-column layout (see CaseStudyView) instead of `position: fixed`
 * chrome pinned over the content. No collapse toggle (removed on request)
 * — always shown. Desktop-only (`hidden md:block`) — mobile has no room
 * for a second column, and CaseStudyView doesn't reserve this column's
 * flex track below `md` either.
 */
export default function CaseStudySideNav({ sections, hasOverview }: CaseStudySideNavProps) {
  ...
  return (
    ...
    <aside className="sticky top-24 hidden max-h-[calc(100vh-7rem)] w-[220px] shrink-0 self-start overflow-y-auto md:block">
      <p className="mb-3 text-[11px] font-semibold tracking-[0.1em] text-fg-secondary uppercase">Table of contents</p>
